                ...
                <div class="sec-content chat-group-full">
                    <div class="scroll-wrapper user-list scrollbar-macosx" style="position: absolute;">
                        <el-select
                            ...
                            remote
                            reserve-keyword
                            placeholder="请输入用户ID/手机/昵称"
                            :remote-method="remoteMethod"
                            :loading="loading"
                            style="width: 100%;margin-bottom: 10px"
                            @change="onSearchMember"
                        >
                            <el-option
                            ...
                            >
                                <span style="float: left">{{ item.nickname }} - {{ item.phone }}</span>
                                <span style="float: right; color: #8492a6; font-size: 13px">
                                    <el-tag size="mini" effect="dark">{{ item.level }}</el-tag>
                                </span>
                            </el-option>
                        </el-select>
...
<script>
    ...
    import { getTeamMember } from '@/api/member'
    ...
    export default {
        ...
        methods: {
            onSend () {
                ...
                let emit
                if (dialogue.object === 1 && dialogue.objectId === 0) {
                    emit = 'sendToSystem'
                } else {
                    emit = dialogue.object === 1 ? 'sendToTeam' : 'sendToGroup'
                }
                this.$store.getters.socket.io.emit(emit, {
            ...
            remoteMethod (query) {
                if (query !== '') {
                    ...
                    setTimeout(() => {
                        this.loading = false
                        this.options = this.teamMembers.filter(item => {
                            console.log(query)
                            return item.phone.indexOf(query) > -1 || item.nickname.indexOf(query) > -1 || item.id === parseInt(query)
                        })
                    }, 200)
                } else {
                    this.options = []
            ...
            onSearchMember (item) {
                ...
                if (this.getDialogueList.find(i => i.objectId === item.id)) {
                    return
                }
                this.$store.dispatch('addDialogueList', {
                    avatar: null,
                    content: '',
                    messageList: [],
                    name: item.nickname,
