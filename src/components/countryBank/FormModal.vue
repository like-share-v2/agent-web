    ...
    <div class="body">
        <el-form :model="mdl" ref="ruleForm" label-width="130px">
            ...
            <el-form-item :label="$i18n.t('BANK_NAME')" required :error="errors.bank_name">
                <el-input v-model="mdl.bank_name" :placeholder="$i18n.t('PLEASE_INPUT', { value: $i18n.t('BANK_NAME') })"></el-input>
            </el-form-item>
            <el-form-item :label="$i18n.t('BANK_ADDRESS')" required :error="errors.bank_address">
                <el-input type="textarea" v-model="mdl.bank_address" :placeholder="$i18n.t('PLEASE_INPUT', { value: $i18n.t('BANK_ADDRESS') })"></el-input>
            </el-form-item>
            <el-form-item :label="$i18n.t('BANK_ACCOUNT')" required :error="errors.bank_account">
                <el-input v-model="mdl.bank_account" :placeholder="$i18n.t('PLEASE_INPUT', { value: $i18n.t('BANK_ACCOUNT') })"></el-input>
            </el-form-item>
            <el-form-item :label="$i18n.t('COUNTRY_BANK_ADDRESS')" required :error="errors.address">
                <el-input type="textarea" v-model="mdl.address" :placeholder="$i18n.t('PLEASE_INPUT', { value: $i18n.t('COUNTRY_BANK_ADDRESS') })"></el-input>
            </el-form-item>
...
<script>
    import { addCountryBank, editCountryBank } from '@/api/countryBank'
        ...
        methods: {
            submitForm () {
                ...
                if (this.mdl.id === 0) {
                    ...
                    editCountryBank(this.mdl.id, this.mdl).then(() => {
                        // 关闭抽屉
                        this.handleCloseDrawer()
                        this.$Message.success(this.$i18n.t('HANDLE_SUCCESS'))
                        // 调用爷爷组件刷新数据
