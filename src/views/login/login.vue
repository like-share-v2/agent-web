...
<!--                            <img src="../../assets/main/logo-dark.png" alt="">-->
                            <div style="font-size: 30px">任务系统</div>
                            <span>{{ siteTitle }}</span>
                        </div>
                        <Form v-if="isLogin" ref="formValidate1" class="login-from" :model="loginJson">
                            <FormItem prop="userName">
<!--                                <el-select v-model="loginJson.add_num" :value="loginJson.add_num">-->
<!--                                    <el-option v-for="(option, index) in countryCodeList" :label="option.name + '（' + option.code + '）'" :value="option.code" :key="index"></el-option>-->
<!--                                </el-select>-->
                                <Input prefix="ios-person-outline" type="text" v-model="loginJson.username" autofocus clearable placeholder="请输入手机号" />
                            </FormItem>
                            <FormItem prop="passWord">
                                <Input prefix="ios-lock-outline" type="password" v-model="loginJson.password" clearable placeholder="请输入密码" />
                            </FormItem>
                            <!--<FormItem prop="code">
                                <Input prefix="ios-lock-outline" type="password" v-model="loginJson.code" clearable placeholder="请输入验证码" />
                            </FormItem>
                            <FormItem>
...
<!--                                <span class="blueText" @click="isLogin=false">注册账户</span>-->
                        ...
                        <Form v-else ref="formValidate2" class="login-from" :model="registerJson">
                            ...
                            <FormItem prop="userName">
                                <Input prefix="ios-person-outline" type="text" v-model="registerJson.account" clearable placeholder="请输入用户名" />
                            </FormItem>
                            <FormItem prop="passWord">
...
<script>
    ...
    import { getCode } from '@/api/default/auth'
    // import { getCountryCodeList } from '@/api/countryCode'
    ...
    export default {
        ...
        methods: {
            ...
            sendMsg () {
                getCode(this.loginJson.username, { add_num: this.loginJson.add_num, type: 6 }).then(() => {
                    this.isDisabled = true
                    const interval = window.setInterval(() => {
                        this.buttonName = '（' + this.time + '秒）后重新发送'
                        --this.time
                        if (this.time < 0) {
            ...
            bgInit () {
                ...
                let width; let height; let largeHeader; let canvas; let ctx; let circles; let target; let animateHeader = true
                // sta
                let lastTime = 0
