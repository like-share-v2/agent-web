    ...
    <div>
        ...
        <div class="table-body">
            ...
            <div class="table-left-top">
                <el-button v-auth="'add'" type="primary" icon="el-icon-plus" @click="onAdd">{{ $i18n.t('CREATE_COUNTRY_BANK') }}</el-button>
...
<!--                <el-table-column align="center" sortable prop="id" label="ID" :width="100" fixed="left"></el-table-column>-->
                ...
                <el-table-column align="center" prop="bank_name" :label="$i18n.t('BANK_NAME')" :min-width="150"></el-table-column>
                <el-table-column align="center" prop="bank_address" :label="$i18n.t('BANK_ADDRESS')" :min-width="150"></el-table-column>
                <el-table-column align="center" prop="bank_account" :label="$i18n.t('BANK_ACCOUNT')" :min-width="150"></el-table-column>
                <el-table-column align="center" prop="address" :label="$i18n.t('COUNTRY_BANK_ADDRESS')" :min-width="150"></el-table-column>
...
<script>
    ...
    import FormModal from '@/components/countryBank/FormModal'
    import { PAGES_SIZE } from '@/config/constants'
    import { getCountryBankList, deleteCountryBank } from '@/api/countryBank'
    ...
    export default {
        ...
        computed: {
            formItems () {
                ...
                return [
                    ...
                    { title: this.$i18n.t('BANK_NAME'), type: 'input', key: 'bank_name' },
                    { title: this.$i18n.t('BANK_ADDRESS'), type: 'input', key: 'bank_address' },
                    { title: this.$i18n.t('BANK_ACCOUNT'), type: 'input', key: 'bank_account' },
                    { title: this.$i18n.t('COUNTRY_BANK_ADDRESS'), type: 'input', key: 'address' }
                ]
            },
            isShowDivider () {
        ...
        methods: {
            ...
            fetch () {
                this.loading = true
                getCountryBankList({ ...this.params, page: this.page.currentPage, perPage: this.page.perPage }).then((res) => {
            ...
            // 删除
            onDelete (ids) {
                deleteCountryBank(ids.join(',')).then(() => {
