    ...
    <div>
        ...
        <div class="table-body">
            ...
            <el-table border style="width: 100%;" stripe :data="data" v-loading="loading" row-key="id" @selection-change="handleSelectionChange">
                ...
                <el-table-column align="center" prop="parent_id" :label="$i18n.t('PARENT_ID')" :width="100">
                    <template slot-scope="scope">
                        <span v-if="scope.row.user !== null">{{ scope.row.user.parent_id }}</span>
                        <span v-else>-</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="ip" label="ip" :min-width="150">
                    <template slot-scope="scope">
                        <span v-if="scope.row.user !== null">{{ scope.row.user.ip }}</span>
                        <span v-else>-</span>
                    </template>
                </el-table-column>
...
<script>
    ...
    import { getTeam, getTeamLevel, getUserLevel } from '@/api/member'
        ...
        computed: {
            formItems () {
                ...
                return [
                    ...
                    { title: this.$i18n.t('USER_MEMBER_LEVEL'), type: 'select_level', key: 'user_level', maxlength: 100, options: this.userLevel },
                    { title: this.$i18n.t('MEMBER_ID'), type: 'input', key: 'user_id', maxlength: 100 }
                ]
            }
        },
        methods: {
