<template>
    <div>
        <el-tabs v-model="activeType" type="card" @tab-click="handleClick">
            ...
            <el-tab-pane :label="$i18n.t('WITHDRAWAL_STATUS_1') + '(' + pass_count + ')'" name="1"></el-tab-pane>
            <el-tab-pane :label="$i18n.t('WITHDRAWAL_STATUS_2') + '(' + refuse_count + ')'" name="2"></el-tab-pane>
        </el-tabs>
        <!-- 搜索组件 -->
        ...
        <div class="table-body">
            ...
            <el-table border style="width: 100%;" stripe :data="data" v-loading="loading">
                <el-table-column align="center" type="expand" :label="$i18n.t('USER_TASK_INFO')"  fixed="left" :width="50">
                    <template slot-scope="props">
                        <el-form label-position="left" inline class="demo-table-expand">
                            ...
                            <el-form-item :label="$i18n.t('MEMBER_NICKNAME')" v-if="props.row.user !== null">
                ...
                <el-table-column align="center" prop="amount" :label="$i18n.t('WITHDRAWAL_AMOUNT')" :min-width="100"></el-table-column>
                <el-table-column align="center" prop="bank_name" :label="$i18n.t('WITHDRAWAL_BANK')" :min-width="100"></el-table-column>
                <el-table-column align="center" prop="name" :label="$i18n.t('WITHDRAWAL_NAME')" :min-width="100"></el-table-column>
                <el-table-column align="center" prop="account" :label="$i18n.t('WITHDRAWAL_ACCOUNT')" :min-width="200"></el-table-column>
                <el-table-column align="center" prop="status" :label="$i18n.t('STATUS')" :min-width="100">
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.status === 0" type="info">{{ $i18n.t('WITHDRAWAL_STATUS_0') }}</el-tag>
                        <el-tag v-else-if="scope.row.status === 1" type="success">{{ $i18n.t('WITHDRAWAL_STATUS_1') }}</el-tag>
                        <el-tag v-else-if="scope.row.status === 2" type="danger">{{ $i18n.t('WITHDRAWAL_STATUS_2') }}</el-tag>
                        <el-tag v-else>-</el-tag>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="created_at" :label="$i18n.t('WITHDRAWAL_TIME')" :min-width="150"></el-table-column>
                <!-- 右侧按钮 -->
...
<script>
    ...
    export default {
        ...
        computed: {
            formItems () {
                ...
                return [
                    ...
                    { title: this.$i18n.t('MEMBER_PHONE'), type: 'input', key: 'user_phone', maxlength: 50 },
                    { title: this.$i18n.t('WITHDRAWAL_ACCOUNT'), type: 'input', key: 'account', maxlength: 50 },
        ...
        methods: {
            ...
            fetch () {
                this.loading = true
                getUserWithdrawalList({ ...this.params, page: this.page.currentPage, perPage: this.page.perPage }).then((res) => {
