<template>
    <div class="body">
        <el-form :model="mdl" ref="ruleForm" label-width="130px">
            <el-form-item :label="$i18n.t('COUNTRY_NAME')" required :error="errors.name">
                <el-input v-model="mdl.name" :placeholder="$i18n.t('PLEASE_INPUT', { value: $i18n.t('COUNTRY_NAME') })"></el-input>
            </el-form-item>
            <el-form-item :label="$i18n.t('COUNTRY_CODE')" required :error="errors.code">
                <el-input v-model="mdl.code" :placeholder="$i18n.t('PLEASE_INPUT', { value: $i18n.t('COUNTRY_CODE') })"></el-input>
            </el-form-item>
            <el-form-item :label="$i18n.t('LANG_NAME')" required :error="errors.lang">
                <el-input v-model="mdl.lang" :placeholder="$i18n.t('PLEASE_INPUT', { value: $i18n.t('LANG_NAME') })"></el-input>
            </el-form-item>
            <!-- 默认添加广告 注释隐藏添加轮播 -->
            ...
            <el-form-item :label="$i18n.t('COUNTRY_IMAGE')" required :error="errors.image">
                <el-upload
                    action=""
...
<script>
    import { addCountry, editCountry } from '@/api/country'
    ...
    export default {
        ...
        methods: {
            ...
            initDefaultFileList () {
                if (this.mdl.image !== '') {
                    this.icon.fileList = [{ name: this.mdl.image, url: configs.staticUrl + this.mdl.image }]
                }
                if (this.mdl.banner !== '') {
            ...
            submitForm () {
                ...
                if (this.mdl.id === 0) {
                    ...
                    editCountry(this.mdl.id, this.mdl).then(() => {
                        // 关闭抽屉
                        this.handleCloseDrawer()
                        this.$Message.success(this.$i18n.t('HANDLE_SUCCESS'))
                        // 调用爷爷组件刷新数据
